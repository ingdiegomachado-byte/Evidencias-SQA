(function () {
	...
	if (!window.hasInjectedContentScript || contextInvalidated) {
		...
		//this my code start
		var capturex_snap_mergedImage_array = [];
		var capturex_snap_mergedImage_index = 0;
		var capturex_capture_top;
		var capturex_capture_bottom;
		var capturex_capture_left;
		var capturex_capture_right;
		var capturex_capture_array = [];
		var capturex_capture_array_width = [];
		var capturex_capture_array_height = [];
		var capturex_capture_array_splicing_index = 0;
		var capturex_canvas_browserMaxHeight_sys = 32767; //Chrome
		var capturex_canvas_browserMaxArea_sys = 268435456;
		var capturex_canvas_browserMaxHeight = capturex_canvas_browserMaxHeight_sys;
		...
		var capturex_com_saveAction = {
			...
			captureVisiblePageScreenshot: function (docHeight, scrollTop, windowInnerHeight, _y1, _y2) {
				...
				setTimeout(function () {
					...
					if (capture_working == 0) {
						...
						if (progressRate > 0)
							chrome.runtime.sendMessage({ action: "setProgress", progress: progressRate });
					}
					else {
						...
						setTimeout(function () {
							...
							setTimeout(function () {
								...
								if ((scrollTop + pageClientHeight) < docHeight && scrollTop < capturex_capture_max_height) {
									...
									chrome.runtime.sendMessage({ action: "setProgress", progress: 100 });
			...
			hideProgressOverlay: function (callback) {
				...
				if (callback) {
					// Esperar a que el navegador redibuje (repainting) el DOM sin la barra
					setTimeout(() => {
		...
		chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
			if (request.action === "checkContentLoaded") {
				...
				chrome.runtime.sendMessage({ action: "requestCaptureScreenshot", y1: request.y1, y2: request.y2 }, function (response) {
					...
					capturex_com_saveAction.restoreProgressOverlay();
